...
DROP FUNCTION IF EXISTS `cantidadIni`;
DELIMITER |
CREATE FUNCTION `cantidadIni`(id int) RETURNS int(11)
BEGIN
...
CREATE ALGORITHM=UNDEFINED SQL SECURITY INVOKER VIEW `kardex` AS select `v`.`fecha` AS `fecha`,if((isnull(`v`.`nFactura`) or (`v`.`nFactura` = '')),`v`.`idPedido`,`v`.`nFactura`) AS `Documento`,`p`.`idProducto` AS `idProducto`,`p`.`nombre` AS `Descripcion`,'Venta' AS `Tipo`,0 AS `Entradas`,sum(`vd`.`cantidad`) AS `Salidas`,`p`.`costo` AS `costo`,`p`.`stock` AS `stock`,`CANTIDADINI`(`p`.`idProducto`) AS `ini` from ((`producto` `p` join `pedido_detalle` `vd` on((`p`.`idProducto` = `vd`.`idProducto`))) join `pedido` `v` on((`v`.`idPedido` = `vd`.`idPedido`))) group by `v`.`idPedido`,`vd`.`idProducto` union all select `c`.`fecha` AS `fecha`,`c`.`nComprobante` AS `Documento`,`p`.`idProducto` AS `idProducto`,`p`.`nombre` AS `Descripcion`,'Compra' AS `Tipo`,sum(`cd`.`cantidad`) AS `Entradas`,0 AS `Salidas`,`p`.`costo` AS `costo`,`p`.`stock` AS `stock`,`CANTIDADINI`(`p`.`idProducto`) AS `ini` from ((`producto` `p` join `compra_detalle` `cd` on((`p`.`idProducto` = `cd`.`idProducto`))) join `compra` `c` on((`cd`.`idCompra` = `c`.`idCompra`))) group by `c`.`idCompra`,`cd`.`idProducto` union all select `a`.`fecha` AS `fecha`,`a`.`idAjuste` AS `Documento`,`p`.`idProducto` AS `idProducto`,`p`.`nombre` AS `Descripcion`,'Ajuste Stock' AS `Tipo`,if((`a`.`tipoAjuste` = 'INCREMENTAR'),`a`.`cantidad`,0) AS `Entradas`,if((`a`.`tipoAjuste` = 'DECREMENTAR'),`a`.`cantidad`,0) AS `Salidas`,`p`.`costo` AS `costo`,`p`.`stock` AS `stock`,`CANTIDADINI`(`p`.`idProducto`) AS `ini` from (`producto` `p` join `ajuste_stock` `a` on((`p`.`idProducto` = `a`.`idProducto`))) group by `a`.`idAjuste`,`a`.`idProducto`;
...
-- Dump completed on 2019-09-22 23:16:32
-- Total time: 0:0:0:1:312 (d:h:m:s:ms)
